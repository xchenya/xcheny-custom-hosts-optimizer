...
const apiAuth = async (c: any, next: any) => {
  ...
  if (isProtectedAPI) {
    ...
    let adminPathAsApiKey = "xchenys-cgj1497756" // 默认值
...
<p>Service is running. Visit /xchenys-cgj1497756 for management.</p>
...
// 管理后台路由
app.route("/xchenys-cgj1497756", admin.use("*", adminAuth))
...
app.get("*", async (c) => {
  ...
  // 检查是否是域名查询路径
  if (path !== "/" && !path.startsWith("/api/") && !path.startsWith("/hosts") && path !== "/favicon.ico" && !path.startsWith("/xchenys-cgj1497756")) {
